...
<body
    class="m-0 font-sans bg-backround-color text-black overflow-x-hidden w-screen dark:bg-gradient-to-r from-gray-800 via-gray-900 to-black dark:text-white transition-colors duration-300">

    {{-- SideBar --}}
    <div id="overlayphone" class="fixed inset-0 w-[100vw] h-[100vh] bg-black bg-opacity-50 hidden z-10"></div>
    <div id="sidebar"
        class="sidebar border-r border-gray-500 p-4 w-[250px] bg-black min-h-screen fixed pt-5 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 z-20">
        ...
        <a class="no-hover block text-white px-8 py-[11px] no-underline text-[18px] hover:bg-[#1a1616]"
            ...
            <img class="block mx-auto mb-5 w-36 cursor-pointer" alt="Logo" height="100"
                src="{{ asset('images/logo-home.webp') }}" width="100" />
        </a>
        <a href="{{ route('home') }}"
    ...
    <div class="relative">
        ...
        <div id="createPlaylistPopup"
            class="hidden fixed inset-0 bg-gray-600 bg-opacity-80 items-center justify-center z-50">
            <div
                ...
                <form id="createPlaylistForm" method="POST" class="space-y-4">
                    ...
                    <input type="text" id="title-playlist" name="title" placeholder="Tiêu đề" required
                        class="w-full px-4 py-3 border border-gray-600 rounded-lg shadow-sm bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:border-x-gray-800" />
    ...
    <div class="main-content pb-[3rem] ml-[100px] p-5">
        <div
            ...
            <button id="toggleSidebar" class="lg:hidden text-black z-50 text-2xl dark:text-white mr-2">☰</button>
            <!-- Search -->
            <form action="{{ route('searchsong') }}" method="get"
                class="relative flex items-center w-full max-w-[20rem] md:max-w-[30rem] lg:max-w-[40rem] pl-8 pr-12"
                id="search-form">
                <div class="relative w-full flex items-center justify-center dark:bg-gray-700 rounded-full bg-gray-500">
                    <input name="query" placeholder="What are you looking for?" type="text" id="query"
                        class="bg-gray-500 text-gray-black rounded-full h-12 w-full text-sm md:text-base pl-12 focus:outline-none placeholder:text-xs md:placeholder:text-sm dark:bg-gray-700 dark:text-white placeholder:text-white transition-colors duration-300" />
                    <button type="submit"
                        class="right-4 mr-2 flex items-center justify-center w-8 h-8 md:w-10 md:h-10 bg-transparent cursor-pointer z-50">
            ...
            @if (Auth::check())
                <div class="flex items-center space-x-2 relative cursor-pointer">
                    ...
                    <div id="popup"
                        class="avatar-popup hidden bg-gray-800 text-white p-4 rounded-lg shadow-lg relative right-0 top-[3.5rem] z-10">
                        <ul class="space-y-2">
                            ...
                            <li class="flex items-center space-x-2">
                                <form action="{{ route('logout') }}" method="POST" style="display: inline;">
                                    @csrf
                                    <button type="submit" class="flex items-center space-x-2 hover:text-gray-300">
            ...
            @else
                ...
                <div class="auth-links flex justify-between items-center cursor-pointer z-200">
                    <a onclick="showLoginForm()"
                        class="text-black text-sm md:text-base no-underline dark:text-white md:p-3 bg-gray-500 px-4 py-2 rounded-full md:px-6 transition-colors duration-300 dark:bg-gray-700">Login</a>
                </div>
            @endif
            <button id="theme-toggle"
                class="flex items-center px-4 py-2 bg-gray-700 text-white font-medium rounded-full hover:bg-gray-600 transition md:flex">
                <span class="icon">
                    <i class="fas fa-sun !hidden dark:block"></i>
                    <i class="fas fa-moon block dark:hidden"></i>
                </span>
                <span class="ml-2 text hidden sm:block">Light</span>
            </button>
