...
<div class="bg-gray-800 p-4 rounded-lg shadow-lg">
    ...
    <div class="mb-4 flex flex-col sm:flex-row gap-4">
        <form action="{{ route('admin.songs') }}" method="GET" class="flex flex-wrap gap-4 items-center w-full">
            <!-- Filter by Song Name -->
            <input type="text" name="song_name" placeholder="Search by song name" 
                value="{{ request('song_name') }}" class="px-3 py-2 bg-gray-700 text-white rounded w-full sm:w-auto">
        ...
        <div class="flex justify-start sm:justify-end w-full sm:w-auto">
            <a href="{{ route('admin.songs') }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 flex items-center justify-center w-full sm:w-auto">
                Clear Filters
            </a>
        </div>
        </form>
                    ...
                    @foreach ($songs as $song)
                        <tr class="border-b border-gray-700">
                            <td class="px-4 py-2 flex justify-center items-center">
                                <img src="/image/{{ $song->img_id }}" alt="Song Image" class="w-10 h-10 rounded-full" loading="lazy">
                            </td>
                            ...
                            <td class="px-4 py-2">
                                ...
                                <form action="{{ route('admin.updateStatus', $song->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <select name="status" onchange="this.form.submit()" class="bg-gray-800 text-white rounded px-2 py-1">
                                            ...
                                            <form action="{{ route('admin.updateSong', $song->id) }}" method="POST" enctype="multipart/form-data">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <textarea id="description" name="description" 
                                                        class="w-full px-3 py-2 bg-gray-700 text-white rounded">{{ $song->description }}</textarea>
                                                </div>
                                                    ...
                                                    <select id="genre_id" name="genre_id" 
                                                        ...
                                                        @foreach ($genres as $genre)
                                                            <option value="{{ $genre->id }}" 
                                                                {{ $song->genre_id == $genre->id ? 'selected' : '' }}>{{ $genre->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
